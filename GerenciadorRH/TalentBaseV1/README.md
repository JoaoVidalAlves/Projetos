...
A pasta [`docs/`](./docs) tem três documentos que se complementam:

- **`ARQUITETURA-TALENTBASE.md`** — modelo de domínio, regras de negócio (pipeline de status, cálculo de experiência, ranking) e mapa de rotas.
- **`STATUS-IMPLEMENTACAO-TALENTBASE.md`** — o que está pronto, o que está mockado e o que falta refinar em cada tela.
- **`GUIA-BACKEND-ENDPOINTS.md`** — endpoint por endpoint que o back-end C#/ASP.NET Core precisa expor, mapeado função a função a partir de `src/services/*.ts`, com exemplos de request/response.
