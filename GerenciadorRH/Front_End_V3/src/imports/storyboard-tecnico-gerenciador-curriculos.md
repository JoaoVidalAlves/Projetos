...
> ⚠️ **DOCUMENTO DESATUALIZADO — substituído por `storyboard-tecnico-gerenciador-curriculos-V2-ATUALIZADO.md`.**
> Esta versão foi escrita antes de uma leitura completa do código-fonte do ZIP e descreve um protótipo bem mais limitado do que o que realmente existe (afirma que só há 4 telas de RH, sem login e sem nenhum fluxo de candidato). Na prática, o código já inclui: autenticação com dois perfis, rotas completas para RH e Candidato, portal público de vagas com candidatura funcional, CRUD de experiências profissionais, e até criação/edição de vaga e ranking de candidatos por vaga (como modais em `VagasPage.tsx`). **Use o documento V2 como referência técnica atual.** Este arquivo é mantido apenas como histórico da análise inicial.
...
> **Atualização:** dos itens abaixo, o código atual **já implementa**: Login/Cadastro, Vitrine pública, Detalhe da vaga + Candidatar-se, Meu Perfil (candidato), Minhas Candidaturas, **Criar/Editar Vaga** (modal `JobModal`) e **Ranking de candidatos por vaga** (modal `RankingPanel`, básico). Só o "Detalhe do Candidato (RH — drill-down)" segue parcial (existe como painel inline, sem histórico/observações). Ver o storyboard V2 para o detalhamento completo e atualizado.
...
> **Ver `storyboard-tecnico-gerenciador-curriculos-V2-ATUALIZADO.md` para o roadmap corrigido** — várias fases aqui descritas como "a fazer" (Fase 3 e parte da Fase 4) já têm implementação funcional no código atual.
