...
**Front-end:** Painel de detalhe inline em `CandidatosPage.tsx` (aberto ao clicar na linha) já mostra dados básicos, skills e score; hoje falta observações e histórico. Uma rota dedicada `/rh/candidatos/:id` (opcional) consumiria este endpoint completo.
...
**Front-end:** Modal `JobModal` (já implementado em `VagasPage.tsx`, botão "Nova vaga") — atualmente chama `createJob()` no contexto mock; falta trocar por este endpoint real. Note que o formulário atual não tem campo de salário (ausente no tipo `Job`).
...
**Front-end:** Mesmo modal `JobModal` (já implementado), reaberto com os dados da vaga preenchidos via `defaultValues`, botão "Editar" nos cards de `VagasPage.tsx` — hoje chama `updateJob()` no contexto mock
...
## 🎯 VagasRankingController — `/api/vagas/{id}/ranking`

Endpoint de ranking. **O front-end já tem um modal funcional (`RankingPanel` em `VagasPage.tsx`)** que hoje calcula tudo no cliente a partir do contexto mock, seguindo a regra "atende exp. mínima primeiro, depois por meses decrescente" (seção 5.3). Este endpoint deve substituir esse cálculo local e adicionar o que falta no front: score de match e habilidades correspondentes/faltantes.

### `GET /api/vagas/{id}/ranking`
**Funcionalidade:** Lista candidatos de uma vaga ordenados por score
