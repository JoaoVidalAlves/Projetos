...
> **Nota de revisão (leitura direta do código):** as tabelas de "Gerenciar Vagas" e "Ranking de Candidatos" foram corrigidas — criar/editar vaga e o ranking básico por experiência **já existem** no protótipo como modais em `VagasPage.tsx`, não são mais itens "não iniciados".
...
| **Detalhe candidato** | ⚠️ Painel inline parcial (sem observações/histórico) | 🔴 Não existe | 🟡 Bloqueado |
| **Adicionar observações** | 🔴 Não existe (nenhum campo hoje) | 🔴 Não existe | 🔴 Crítico |
| **Excluir/arquivar** | ✅ Excluir funcional (mock); "Arquivar" não existe | 🔴 Não existe | 🟡 Bloqueado |
...
| **Criar vaga** | ✅ Modal `JobModal` funcional (mock) | 🔴 Não existe | 🟡 Bloqueado |
| **Editar vaga** | ✅ Modal `JobModal` funcional (mock) | 🔴 Não existe | 🟡 Bloqueado |
| **Ranking de candidatos por vaga** | ✅ Modal `RankingPanel` funcional (mock, sem score de habilidades) | 🔴 Não existe | 🟡 Bloqueado |
| **Ver detalhe** | ✅ Layout pronto | 🔴 Não existe | 🟡 Bloqueado |
| **Encerrar vaga** | ✅ Já é possível via `JobModal` (campo status) | 🔴 Não existe | 🟡 Bloqueado |
...
| Funcionalidade | Front-end | Back-end | Status |
|---|---|---|---|
| **Tela/modal de ranking** | ✅ Modal `RankingPanel` funcional (mock) em `VagasPage.tsx` | 🔴 Não existe | 🟡 Bloqueado |
| **Cálculo de score (experiência)** | ✅ Implementado (Caminho B: qualifica + ordena por meses) | 🔴 Não existe | 🟡 Bloqueado |
| **Score de match com habilidades** | 🔴 Não existe (ranking só considera experiência) | 🔴 Não existe | 🔴 Pendente |
| **Filtro por habilidades** | 🔴 Não existe | 🔴 Não existe | 🔴 Pendente |

**Impacto bloqueador:** NÃO — o ranking básico já funciona no front (mock); falta conectar a um endpoint real e enriquecer com habilidades.
...
│ /cadastro        ✅ Form + LGPD 100%         🔴 Não existe      POST /registrar
│ /                ✅ UI + filtros 100%        🔴 Não existe      GET /vagas?open
│ /vaga/:id        ✅ UI + candidatar-se 95%   🔴 Não existe      GET /vagas/{id}
│ /rh/dashboard    ✅ Funcional 100% (mock)    🔴 Não existe      GET /dashboard/*
│ /rh/candidatos   ✅ Tabela + detalhe 90%     🔴 Não existe      GET /candidatos
│ /rh/vagas        ✅ Grid + CRUD + Rank. 90%  🔴 Não existe      GET /vagas (RH)
...
│ Ranking (modal)  ✅ Básico (só exp.) 60%     🔴 Não existe      Score c/ habilidades
│ Detalhe Cand.    ⚠️ Painel inline 70%        🔴 Não existe      Observações+histórico
│ Criar/Edit Vaga  ✅ Modal 85%                🔴 Não existe      Conectar API + salário
└────────────────────────────────────────────────────────────────────────┘
```
