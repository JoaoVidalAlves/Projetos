...
#### Vagas + Candidatos
✅ CRUD de vagas (criar, editar, listar) — modal `JobModal` em `VagasPage.tsx`, já chama `createJob()`/`updateJob()`
✅ Ranking de candidatos por vaga — modal `RankingPanel` em `VagasPage.tsx`, já ordena por "atende requisito" + meses de experiência
✅ Cadastro manual de candidatos (RH) — formulário com 2 seções (dados pessoais + profissionais); **não tem seção de observações**
✅ Filtros e buscas estruturados (inclui filtro por departamento na vitrine pública, que já funciona)
⚠️ Painel de detalhe de candidato (RH) — existe como painel lateral inline em `CandidatosPage.tsx`, mas sem rota própria, sem campo de observações e sem histórico de status

**Impacto:** A maior parte da regra de negócio já existe no front, **incluindo telas que a análise anterior listava como faltantes** (criar/editar vaga, ranking). O back-end precisa validar e persistir, não reinventar.
...
| **Completar painel de detalhe do candidato (RH)** | Painel já existe (inline em `CandidatosPage.tsx`) | 2–3 dias | Falta campo de observações do RH e histórico de mudanças de status; avaliar se vale virar rota própria |
| **Enriquecer ranking de candidatos por vaga (RH)** | Modal já existe (`RankingPanel` em `VagasPage.tsx`) | 3–4 dias | Falta score de match (%), tags de habilidades e filtros — a ordenação básica por experiência já funciona |
...
```json
{
  ...
  "lucide-react": "latest",
  "sonner": "latest",
  "react-hook-form": "7.55.0", // ✅ Já em uso (Login, Cadastro, Cadastrar, modal de Vaga, Perfil)
  "date-fns": "3.6.0", // ✅ Já no package.json (ainda não usado no código)
  "zod": "❌ NÃO está no package.json — precisa ser adicionado",
  "axios": "❌ ADICIONAR",
...
│   │   ├── CadastroPage.tsx    ⚠️ Form + LGPD prontos, sem API
│   │   ├── VitrinePage.tsx     ✅ Filtros funcionais (cargo, depto, modalidade, tipo), dados mockados
│   │   ├── VagaDetailPage.tsx  ✅ Rota `/vaga/:id` funcional, "Candidatar-se" já cria Candidatura real no contexto
│   │   └── rh/
│   │       ├── DashboardPage.tsx    ✅ Pronto (mock; gráfico usa série fixa, não recalculada)
│   │       ├── CandidatosPage.tsx   ⚠️ Tabela + painel de detalhe inline prontos, dados mockados
│   │       ├── VagasPage.tsx        ✅ Grid + CRUD (modal) + Ranking (modal) prontos, dados mockados
│   │       └── CadastrarPage.tsx    ✅ Pronto (mock), 2 seções (sem "observações")
│   └── layouts/
│       ├── PublicLayout.tsx
│       ├── RhLayout.tsx
│       └── CandidatoLayout.tsx
