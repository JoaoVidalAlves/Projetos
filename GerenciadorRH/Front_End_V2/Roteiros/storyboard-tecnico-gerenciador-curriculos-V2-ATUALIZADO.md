...
| **Candidatos RH** (`/rh/candidatos`) | ⚠️ **Estrutura pronta, lógica parcial** | Tabela com avatar, nome, vaga, depto., local, experiência, status (dropdown editável), data, ações (excluir). Filtros por status/depto. Busca por texto. |
| **Vagas RH** (`/rh/vagas`) | ⚠️ **Estrutura pronta, lógica parcial** | Grid de cards com título, depto., tipo (CLT/PJ), status, local, data, nº de vagas, nº candidatos, avatares stack |
...
|---|---|---|
| **Detalhe Candidato (RH)** | Ainda não tem rota específica | Tela drill-down mostrando: dados completos, experiências, histórico de status, observações do RH, ações (mudar status, arquivar) |
| **Criar/Editar Vaga (RH)** | Função `createJob()` e `updateJob()` no contexto | Form para criar/editar vaga com validação; roteamento a `/rh/vagas/:id/editar` |
| **Ranking de Candidatos por Vaga (RH)** | Lógica de ranking existe na seção 5; função `getRanking()` — **ainda não implementada** | Tela `/rh/vagas/:id/ranking` com tabela ordenada, score de match, filtros avançados |
...
- Atualizar formulários (`LoginPage`, `CadastroPage`) para usar `react-hook-form` + `zod` (já no `package.json`)
...
**Front-end:**
31. Criar tela **Ranking de Candidatos** (`/rh/vagas/:id/ranking`) mostrando score, matching de habilidades, ordenação
32. Melhorar formulários com `react-hook-form` + `zod` (validação tipada)
33. Adicionar `date-fns` para manipulação de datas (experiência: início/fim)
...
| `/rh/candidatos` | `CandidatosPage` | ⚠️ Tabela + filtros prontos, **carrega `INITIAL_CANDIDATES` mockado** |
| `/rh/vagas` | `VagasPage` | ⚠️ Grid de cards + filtros prontos, **carrega `INITIAL_JOBS` mockado** |
...
- ✅ Aplicar a vaga (Candidato) → cria `Candidatura` no contexto, incrementa contador de candidatos
...
- 🔴 Upload de currículo em PDF — campo pronto, mas sem integração com storage
- 🔴 Persistência de dados — atualizar página perde tudo (normal para mock)
- 🔴 Tela de detalhe de candidato (RH) — não existe rota
- 🔴 Tela de criar/editar vaga — funções existem no contexto, mas sem UI dedicada
- 🔴 Tela de ranking por vaga — lógica não implementada
- 🔴 Busca/filtros avançados em alguns lugares (ex.: filtro por departamento na vitrine)
